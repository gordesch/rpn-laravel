...
<div class="flex-grow focus-within:z-10">
  <input id="i_{{ $show->id }}_poster" class="form-input block w-full rounded-none rounded-l-md transition ease-in-out duration-150 sm:text-sm sm:leading-5 text-gray-400 bg-gray-50 shadow-inner" value="{{ $show->getFirstMediaUrl('posters', 'sm@2x') }}" readonly />
</div>
<button data-clipboard-target="#i_{{ $show->id }}_poster" class="copy-btn -ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-r-md text-gray-700 bg-gray-50 hover:text-gray-500 hover:bg-white focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
  ...
  <label class="ml-2 cursor-pointer" for="i_{{ $show->id }}_poster">Affiche</label>
</button>
