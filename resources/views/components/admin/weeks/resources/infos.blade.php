...
<div class="flex-grow focus-within:z-10">
  <textarea id="i_{{ $programming->show->id }}_infos" class="form-textarea block w-full rounded-none rounded-l-md transition ease-in-out duration-150 sm:text-sm sm:leading-5 text-gray-400 bg-gray-50 shadow-inner" rows="1" style="min-height: 38px;"readonly>{{ $programming->show->genre_director_country_year_string}}
{{ $programming->show->synopsis }}</textarea>
</div>
<button data-clipboard-target="#i_{{ $programming->show->id }}_infos" class="copy-btn -ml-px relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm leading-5 font-medium rounded-r-md text-gray-700 bg-gray-50 hover:text-gray-500 hover:bg-white focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150">
  ...
  <label class="ml-2" for="i_{{ $programming->show->id }}_infos">Infos</label>
</button>
