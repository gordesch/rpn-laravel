<x-admin.layout title="Programmation" category="programmings">
  ...
  <form id="form" method="POST" action="{{ route('admin.weeks.update', [$week]) }}">
    @method('PUT')
    @csrf
    <ul id="programmings" class="grid xl:grid-cols-3 sm:grid-cols-2 grids-cols-1 grid-flow-row col-gap-2 row-gap-4 focus:outline-none">
      @foreach($week->programmings as $programming)
        <li x-data="{ open: false, html: {{ $programming->custom_showings_infos ? 'true' : 'false' }} }" id="programming_{{ $programming->id }}" class="min-w-0 col-span-1 bg-white hover:bg-gray-50 shadow focus:outline-none focus:shadow-outline-indigo sm:rounded-md cursor-move">
          <div class="block ">
                  ...
                  <div>
                    <div class="sm:flex sm:items-center sm:justify-between">
                      ...
                      <div class="flex items-center flex-shrink-0 text-xs leading-5">
                        <div class="flex-shrink-0 py-1 pl-2 pr-1 border border-gray-100 bg-gray-100 rounded-l-md text-gray-700 text-xs">
                          {{ $programming->showings_count > 1 ?  $programming->showings_count . ' séances' : $programming->showings_count . ' séance'}}
                        </div>
                        <button @click.prevent="open = !open; $nextTick(() => { $refs.html_input.focus(); });" type="button" class="flex items-center py-1 px-2 border border-gray-300 rounded-r-md shadow-sm font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800 transition duration-150 ease-in-out">
                          <span :class="{ 'bg-gray-200': !html, 'bg-indigo-600': html }" class="relative inline-block flex-no-shrink mr-1 -mt-px mb-px h-3 w-5 border-2 border-transparent rounded-full transition-colors ease-in-out duration-200 focus:outline-none focus:shadow-outline {{ $programming->custom_showings_infos ? 'bg-indigo-600' : 'bg-gray-200' }}">
                            <span aria-hidden="true" :class="{ 'translate-x-2': html, 'translate-x-0': !html }" class="inline-block h-2 w-2 rounded-full absolute top-0 left-0 bg-white shadow transform transition ease-in-out duration-200 {{ $programming->custom_showings_infos ? 'translate-x-2' : 'translate-x-0' }}"></span>
                          </span>
                          <span>
                            HTML
                          </span>
                    ...
                    <div x-show="open === true" style="display: none;" @click.away="open = false" class="flex my-px rounded-md shadow-sm"">
                    ...
                    <div x-show="!open" class="grid grid-rows-2 grid-cols-2 gap-2 md:flex md:justify-between md:items-center mt-2 text-xs font-medium leading-5">
