<tr
  @if (!$show->ignore_missing && (!$show->duration_in_seconds || !$show->genre || !$show->synopsis || !$show->videos_count))
  class="bg-red-50"
  @elseif (!$show->ignore_missing && (!$show->country || !$show->year || !$show->director || !$show->cast))
  class="bg-yellow-50"
  @endif
>
  ...
  <td class="whitespace-no-wrap border-b border-gray-200">
    ...
    @else
      <x-admin.weeks.shows-state.missing-important :href="route('admin.shows.edit', [$show])" :ignoreMissing="$show->ignore_missing"/>
    @endif
  </td>
  <td class="whitespace-no-wrap border-b border-gray-200">
  ...
  <td class="whitespace-no-wrap border-b border-gray-200">
    @if ($show->country)
      <x-admin.weeks.shows-state.valid />
    @else
      <x-admin.weeks.shows-state.missing :href="route('admin.shows.edit', [$show])" :ignoreMissing="$show->ignore_missing"/>
    @endif
  </td>
  <td class="whitespace-no-wrap border-b border-gray-200">
    @if ($show->year)
  ...
  <td class="whitespace-no-wrap border-b border-gray-200">
    ...
    @else
      <a
        ...
        >
        <x-admin.weeks.shows-state.missing-important :ignoreMissing="$show->ignore_missing" />
      </a>
  ...
  <td class="whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium">
    @if ($show->shows_provider_id)
      <button
        ...
        type="button"
        @if (!$show->ignore_missing && (!$show->duration_in_seconds || !$show->genre || !$show->synopsis || !$show->country || !$show->year || !$show->director || !$show->cast))
  ...
  <td class="whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium">
    <button wire:click.prevent="toggleIgnoreMissing" type="button" class="inline-flex items-center mr-4 px-3 py-2 border border-gray-300 text-xs leading-4 font-medium rounded-md shadow-sm bg-white @if (!$show->ignore_missing) text-gray-500 hover:text-gray-400 @else text-gray-700 hover:text-gray-500 @endif focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:text-gray-600 active:bg-gray-50 transition ease-in-out duration-150">
      <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        @if ($show->ignore_missing)
